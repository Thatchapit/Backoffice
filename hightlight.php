      ...
      <div class="row wrapper border-bottom page-heading">
        <div class="col-lg-12">
          ...
          <ol class="breadcrumb">
            ...
            <li  class="active">  <strong><a>Hightlight</a> </strong> </li>
              ...
              <div class="ibox-content collapse in">
                <div class="widgets-container">
                   <div class="align-right">
                             <a href="hightlightsetting.php"><button type="button" class="btn btn-primary waves-effect">
                              สร้างหัวข้อใหม่
                             </button></a>
                            </div>
                            <br/>
                                    ...
                                    <tbody>
                                     ...
                                     <?php
                                        ...
                                        ?>
                                          ...
                                          <tr>
                                            <td><?php echo  "0 (HIGHTLIGHT)" ?></td>
                                            <td><img src="images/hightlight/desktop/<?php echo $result['desktop_img'] ?>" alt="" height="150"></td>
                                             <td><?php echo $result['title'] ?></td>
                                            <td><?php echo $result['detail'] ?></td>
                                            <td><?php echo $result['type'] == 0 ? "HightLight" : ""  ?></td>
                                            <td><?php echo $result['create_date'] ?></td>
                                            <td>   <div class="form-group">
                                                <?php if($result['is_hightlight'] == 1){
                                                $check = 'checked="checked"';
                                            }else{
                                                 $check = '';
                                            } ?>
                                                <input type="checkbox" id="checkbox" class="check_hightlight" data-id="<?php echo $result['id'] ?>"  <?php echo $check ?> disabled>
                                                <label for="checkbox"></label>
                                            </div></td>
                                            <td> <div class="form-group">
                                            
                                                <input type="checkbox" id="checkbox_random" class="check_random" data-id="<?php echo $result['id'] ?>"  disabled>
                                                <label for="checkbox_random"></label>
                                            </div></td>
                                            <td><a href="hightlightsetting.php?id=<?php echo  $result['id'] ?>"><button type="button" class="btn btn-warning waves-effect">แก้ไข</button></a>&nbsp;&nbsp;<button type="button" data-type="cancel" data-id="<?php echo  $result['id'] ?>" class=" del_btn btn btn-danger waves-effect">ลบ</button></td>
                                        ...
                                        $db->where ("is_hightlight", 0);
                                            ...
                                            $results = $db->get('hightlight');
                                            for ($i=0; $i < count($results) ; $i++) { 
                                        ?>
                                
                                        <tr>
                                            <td><?php echo  $i+1 ?></td>
                                            <td><img src="images/hightlight/desktop/<?php echo $results[$i]['desktop_img'] ?>" alt="" height="150"></td>
                                            <td><?php echo $results[$i]['title'] ?></td>
                                            <td><?php echo $results[$i]['detail'] ?></td>
                                            <td><?php echo $results[$i]['type'] == 0 ? "HightLight" : ""  ?></td>
                                            <td><?php echo $results[$i]['create_date'] ?></td>
                                            <td>   <div class="form-group">
                                                <?php if($results[$i]['is_hightlight'] == 1){
                                                $check = 'checked="checked"';
                                            }else{
                                                 $check = '';
                                            } ?>
                                                <input type="checkbox" id="checkbox_<?php echo $i?>" class="check_hightlight" data-id="<?php echo $results[$i]['id'] ?>"  <?php echo $check ?>>
                                                <label for="checkbox_<?php echo $i?>"></label>
                                            </div></td>
                                            <td> <div class="form-group">
                                                <?php if($results[$i]['is_random'] == 1){
                                                $check = 'checked="checked"';
                                            }else{
                                                 $check = '';
                                            } ?>
                                                <input type="checkbox" id="checkbox_random_<?php echo $i?>" class="check_random" data-id="<?php echo $results[$i]['id'] ?>"  <?php echo $check ?>>
                                                <label for="checkbox_random_<?php echo $i?>"></label>
                                            </div></td>
                                            <td><a href="hightlightsetting.php?id=<?php echo  $results[$i]['id'] ?>"><button type="button" class="btn btn-warning waves-effect">แก้ไข</button></a>&nbsp;&nbsp;<button type="button" data-type="cancel" data-id="<?php echo  $results[$i]['id'] ?>" class=" del_btn btn btn-danger waves-effect">ลบ</button></td>
...
<script>
  ...
  $('.check_hightlight').change(function() {
       var id = $(this).data('id');
       $(this).prop("checked", false);
       showConfirmMessage_hightlight(id);
       
    });


  $('.check_random').change(function() {
       var id = $(this).data('id');
     ...
     if($(this).is(":checked")) {
      random_status = 1;
                ...
                $.ajax({
                    url: url ,
                    data: { 'mode' : 'random','id' : id,'random_status': random_status},
                    type: 'POST',
 ...
 function showConfirmMessage_hightlight(id) {
    swal({
        title: "คุณแน่ใจที่จะเปลี่ยน HightLight",
        text: "คุณต้องการที่จะใช้หัวข้อนี้เป็น HightLight ใช่ไหม",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        confirmButtonText: "Yes, it HightLight!",
        closeOnConfirm: false
    }, function () {
                        var url = "ajax/ajax_hightlight.php"; // the script where you handle the form input.
