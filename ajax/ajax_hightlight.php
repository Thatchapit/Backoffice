...
date_default_timezone_set("Asia/Bangkok");
    ...
    $data = Array ("is_del" => 1);
     ...
     $db->where ('id', $id);
            if ($db->update ('hightlight', $data))
                echo $db->count . ' records were updated';
            else
                echo 'update failed: ' . $db->getLastError();
       }
       else if($mode == "hightlight"){
           $data = Array ("is_hightlight" => 0);
            $db->update ('hightlight', $data);

            $data = Array ("is_hightlight" => 1);
             $db->where ('id', $id);
            if ($db->update ('hightlight', $data))
                echo $db->count . ' records were updated';
            else
           ...
           $ranom_status = 1;
            if(isset($_POST['random_status'])){
              $ranom_status = $_POST['random_status'];
            }
           
            $data = Array ("is_random" => $ranom_status);
             $db->where ('id', $id);
            if ($db->update ('hightlight', $data))
                echo $db->count . ' records were updated';
            else
    ...
    $TARGET_PATH_DESKTOP = "../images/hightlight/desktop/";
    $TARGET_PATH_MOBILE = "../images/hightlight/mobile/";
    $imagename = "";
    $image_mobile_name = "";
    ...
    if(isset($_FILES['image_mobile-file'])&& $_FILES['image_mobile-file']['name'] != ""){
        ...
        if($mode == "add"){
            if($imagename != ""){
                ...
                $editor->save( $image1, "../images/hightlight/mobile/m_".$imagename);
